...
def getContours(img,area_min,area_max,outfile,white_threshold):
    ...
    Q2 = 400
    Q3 = 550
    Q4 = 630    #635 image 20 et 630 image 21
    exception1 = array([3, 4, 5, 6, 7])  # METTRE LE NUMERO DE PLACE MANQUANT 3, 4, 5, 6, 7
    exception2 = array([1])   #2, 6 image 20 et 1 image 21
    exception34 = array([0])  # PLACE HANDICAPÉ   #11110 image 20 et 0 image 21
    ...
    print("on a distance moyenne =", averagedist1, som)
    ...
    print("on a distance moyenne =", averagedist2, som)
    ...
    print("on a distance moyenne =", averagedist3, som)
    ...
    for v in range(20):  # taille à modifier SI TROP LONG CALCUL TROP DE BOUCLES OU SI RESULTAT PAS LINEAIRE
        ...
        for j in range(c1.size // 2-1):
            ...
            if j != 100000:  # un watchdog
                if distance < 60 and distance > 20 and h < 10:
                    print("good1")

                else:
                    l = [c1[j, 0] - averagedist1 +15,c1[j, 1]-1]  # IMPORTANT ICI SELON ZOOM DE L'IMAGE +15 ou +12 change tout
                    c1 = np.insert(c1, j + 1, l, axis=0)
                    print("test ifff 4 BOUCLE 2", j, v)
                    break
            else:
                print("exception1")

        # print("TESTOOOOOO c2222222222 COMPTEUR", c2.shape, c1.shape)
        for j in range(c2.size // 2):
            ...
            if j != 100000:
                if distance < 60 and distance > 20 and h < 10:
                    ...
                    print("test ifff 4 BOUCLE 1", j, v)

            else:
                print("exception1")

    print("IMPORTANT TAILLE C3", c3.size)
    ##### POUR C3 C4
    print(" COMPTEUR 2", c1.shape, c2.shape, n4)
    ...
    for v in range(20):
        for j in range(c4.size // 2-1):
            ...
            print("H TESTOOOO", h)
            if j != 100000:
                if distance < 60 and distance > 20 and h < 20:
                    print("good1")

                else:
                    cnt4 = cnt4 + 0.5  # cnt4 +0.5 pour image 21 et 20
        ...
        for j in range(c3.size // 2):
            ...
            if j != 100000:
                if distance < 60 and distance > 20 and h < 20:
                    ...
                    cnt3 = cnt3+0.7  # cnt3+0.7 pour image 21 et -3 pour image 20
    ...
    # print("TESTOOOOOO c2222222222 COMPTEUR", c1.shape, c2.shape, n4)
    q = 0
    for j in range(compt4+1):  #compt4+1 pour image 21 et compt4 pour image 20
        if j != exception2[q]:
            if j % 2 == 0:
                colour = (255, 255, 0)
            else:
    ...
    k = 0
    for j in range(compt2):  # compt2+1 pour image 20 et compt2 pour image 21
        if j != exception1[k]:
            if j % 2 == 0:
                colour = (255, 255, 0)
            else:
    ...
    print(compt2, c3.size, c2.size)
    #print(exception1[1], exception1[2], exception2)

    cv2.imshow("TESSSST3", imgContour)
    cv2.imshow("imgContour3", imgContour3)
    # cv2.imshow("TESSSST2", imgf)
    #cv2.imshow("TESSSST1", dst)
