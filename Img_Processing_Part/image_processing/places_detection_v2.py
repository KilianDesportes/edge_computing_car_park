...
def getContours(img,area_min,area_max,outfile,white_threshold):
    ...
    exception2 = array([2, 6])   #2, 6
    exception3 = array([5])  # PLACE HANDICAPÉ
    ...
    for v in range(20):
        for j in range(c4.size // 2-1):
            ...
            #print("H TESTOOOO", h)
            if j != 100000:
                if distance < 60 and distance > 20 and h < 40:
                    #print("good1")
                    pass
                else:
                    l = [c4[j, 0] - averagedist4+10, c4[j, 1]]
        ...
        for j in range(c3.size // 2):
            ...
            if j != 100000:
                if distance < 60 and distance > 20 and h < 40:
                    ...
                    l = [c3[j, 0] - averagedist3 -3, c3[j, 1]-1]
                    c3 = np.insert(c3, j + 1, l, axis=0)
                    # print("test ifff 4444 BOUCLE 2", j,v)
                    break
            else:
                pass
                #print("exception2")
    ...
    k = 0
    for j in range(compt2+1):  # c2.size//2-1  prendre compt plutot
        if j != exception1[k]:
            if j % 2 == 0:
                colour = (255, 255, 0)
            else:
    ...
    """
    img3 = cv2.imread("test_input/images/img20.png")
    img4 = cv2.resize(img3, (800, 400))

    imgf = cv2.resize(imgContour3, (800, 400))

    dst = cv2.addWeighted(imgf, 0.3, img4, 0.7, 0)
